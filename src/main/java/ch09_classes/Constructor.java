...
/*
    ...
    특징 :
        1. 클래스의 이름과 생성자의 이름은 동일 (다른 메서드와 달리 대문자로 시작/ 명사일 확률이 높음)
        2. return 값이 없다 -> call1() / call2() 유형
        3. 객체 초기화 : 생성자는 객체의 필드 초기화를 담당

    생성자 정의 방식
        1. 기본 생성자 : 매개변수가 없는 생성자
            (사실 클래스를 만들 때 default로 생성되어 정의하지 않을 때가 있음
             -> 저희는 Car 클래스에서 생성자를 정의한 적 없지만 CarMain 에서 객체를 생성할 수 있었던 점에서 알 수 있음
...
public class Constructor {
    ...
    // 매개변수 생성자 # 2
    Constructor(String title){  // 추후에는 매개변수명과 속성명을 일치시킬 예정
        this.name = title;
