...
//        animalData3.printData();
        /*
            23번 라인에서 오류가 생긴다는 점을 확인
            AnimalData 클래스의 .printData() 메서드를 호출하는 과정에서
            Car 객체(AnimalData의 필드인)를 강제로 Animal 클래스로 형변환하려고 했기 떄문에 발생하는 예외인데
            결국 필드에 Car 객체가 대입은 되지만 메서드는 호출되지 않는다는 점을 확인

            그렇다면 애초에 Car 객체가 필드에 대입되지 못하도록 설정할 필요성이 대두되었는데,
            이 것이 와일드 카드 개념의 의의 입니다.
         */
